...
> **2026-09-13 덧붙임.** 위 13장과 별개로, 자동완성 수요가 실측된 지역에는 `misul`
> (미술학원) 과목 페이지를 따로 세웠다 — 수원 · 구래동 · 장기동 · 운양동 · 상동 ·
> 부평 · 송파. 이 페이지들은 그 지역만의 `intro` · `points` 를 채워 자기 canonical 을
> 유지한다(겹침 71~80%). 위 표의 13장은 여전히 대표 페이지로 합쳐진 상태이고,
> 같은 방식으로 내용을 채우면 되살아난다. 측정값은 `content/KEYWORDS.md` 참고.

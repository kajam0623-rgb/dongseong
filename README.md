...
노코드 웹사이트 빌더 SaaS MVP. 빌드 과정 없이 `사이트온-빌더.html`을 브라우저에서 열면 실행됩니다.

- 디자인 스타일 12종 × 컬러 테마 10종 + 커스텀 컬러
...
- 의료법 준수 (치료후기·전후사진 없음)
- localStorage 자동저장 + JSON 프로젝트 파일, 독립 HTML 내보내기
...
`tests/golden.html`을 브라우저에서 열면 골든 테스트가 실행됩니다. 빌더를 로드한 뒤 12개 스타일 전체에 대해 `generateSite(getConfig())`를 실행하고, 필수 섹션 id, 문서 경계, 이모지 누출, 미리보기 렌더, 콘솔 에러를 확인합니다.
